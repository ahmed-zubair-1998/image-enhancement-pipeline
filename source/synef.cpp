...
vector<double> get_regions(Mat *prev) {
	...
	*/
	...
	Mat yuv;
	cvtColor(small, yuv, COLOR_RGB2HSV);
...
void synEF(Mat &prev, Mat &res) {
	...
	vector<Mat> pme;		//vector containing images for exposure fusion.
	...
	for (int i = 0; i < 3; i++) {
		...
		//string t_name = "../results/test_gamma_" + to_string(i) + ".jpg";
		//imwrite(t_name, ptemp);
		...
		//t_name = "../results/test_contr_" + to_string(i) + ".jpg";
		//imwrite(t_name, res);
	}
